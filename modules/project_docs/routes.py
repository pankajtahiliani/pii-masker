...
# Shared label map — used by both refine_artifact and download_docx
_ARTIFACT_LABELS = {
...
def generate_artifact():
    ...
    try:
        t = profile["timeout"]
        logger.info("[artifact] key=%s model=%s src_len=%d max_tokens=%d timeout=%ds",
                    artifact, model, len(src_short), predict, t)
        ...
        logger.info("[artifact] key=%s done result_type=%s", artifact, type(result).__name__)
        ...
        logger.error("[artifact] %s: %s", artifact, e, exc_info=True)
...
def refine_artifact():
    ...
    src_short       = source[:3000].strip()
    label           = _ARTIFACT_LABELS.get(artifact, artifact.replace('_', ' ').title())
    ...
    try:
        ...
        logger.info("[refine] key=%s model=%s predict=%d timeout=%ds", artifact, model, predict, t)
        ...
        logger.error("[refine] %s: %s", artifact, e, exc_info=True)
        return jsonify({"error": "Internal server error"}), 500
...
def generate_docs():
    ...
    for key in artifacts:
        ...
        try:
            ...
            logger.error("[generate-docs] %s failed: %s", key, e)
...
def download_docx():
    ...
    try:
        ...
        for key, artifact_data in data.items():
            label = _ARTIFACT_LABELS.get(key, key.replace('_', ' ').title())
        ...
        logger.error("[download-docx] %s", e, exc_info=True)
        return jsonify({"error": "Internal server error"}), 500
